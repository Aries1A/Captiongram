...
$(function() {
  const textarea = $.cookie('Exiftext');
  $('#textExif').val(textarea);

  $('#caption2').on('click', function() {
    $.cookie('Exiftext', document.querySelector('#textExif').value, {
      expires: 730
    });
    console.log($.cookie('Exiftext'));
    const file_input = $('#file-input');
    const file = file_input[0].files[0];
    ...
    EXIF.getData(file, function() {
      ...
      try {
        ...
        const Date = DateTimeOriginal.substr(0, 4) + '/' + DateTimeOriginal.substr(5, 2) + '/' + DateTimeOriginal.substr(8, 2);
        // $('#textExif').val(Date);
        let str = document.getElementById('textExif').value;
      ...
      try {
        const Length = EXIF.getTag(this, 'FocalLengthIn35mmFilm') + 'mm';
        let str = document.getElementById('textExif').value;
      ...
      try {
        let second = 0;
        if (EXIF.getTag(this, 'ExposureTime') < 1) {
          second = 1 / EXIF.getTag(this, 'ExposureTime') + 's';
        } else {
          second = EXIF.getTag(this, 'ExposureTime') + 's';
        }
        let str = document.getElementById('textExif').value;
        str = str.replace('[!Time]', second);
        $('#textExif').val(str);
      } catch (error) {
        alert('露光時間情報がありません');
      }
      ...
      try {
        const FNumber = 'f/' + EXIF.getTag(this, 'FNumber');
        let str = document.getElementById('textExif').value;
        str = str.replace('[!FNumber]', FNumber);
        $('#textExif').val(str);
      } catch (error) {
        alert('F値情報がありません');
      }
      ...
      try {
        const ISO = 'ISO' + EXIF.getTag(this, 'ISOSpeedRatings');
        let str = document.getElementById('textExif').value;
        str = str.replace('[!ISO]', ISO);
        $('#textExif').val(str);
      } catch (error) {
        alert('ISO情報がありません');
      }
